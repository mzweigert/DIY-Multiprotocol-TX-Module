...
void setup()
{
  ...
  { // PPM
#ifndef MY_PPM_PROT
    const PPM_Parameters *PPM_prot_line = &PPM_prot[mode_select - 1];
#else
    const PPM_Parameters *PPM_prot_line = &My_PPM_prot[mode_select - 1];
#endif
...
#ifdef ENABLE_PPM
int8_t init_modeselect() {
  int8_t mode = eeprom_read_byte((EE_ADDR)MODELMODE_EEPROM_OFFSET);
  if (mode > NBR_MODES) {
    eeprom_write_byte((EE_ADDR)MODELMODE_EEPROM_OFFSET, 0x01); // set bank to 0
    mode = 1;
  }
  phase = 3;
  blink = millis();
  ...
  DOWN_BTN_init;
  LED_output;
  bool up_or_down_btn_pressed = false;
  byte blink_counter = 0;
  while (!IS_BIND_BTN_on)
  { //loop here if the dial is on position 15 for user to select the bank
    if (blink < millis())
    {
      switch (phase)
      { // Flash mode number of times
        case 0:        
          LED_on;
          blink = millis() + BLINK_MODE_TIME_HIGH;
          phase++;
          break;
        case 1:
          LED_off;
          blink = millis() + BLINK_MODE_TIME_LOW;
          phase++;
          break;
        case 2:
          blink_counter++;
          phase = 0;
          if (blink_counter >= mode) {
            blink = millis() + BLINK_MODE_REPEAT;
            blink_counter = 0;
          } else
            blink = millis() + BLINK_MODE_TIME_LOW;
          break;
    ...
    if (IS_UP_BTN_on && IS_DOWN_BTN_on) {
      up_or_down_btn_pressed = true;
      continue;
    } else if ((IS_UP_BTN_on || IS_DOWN_BTN_on) && !up_or_down_btn_pressed) {
      LED_on;
      if (IS_UP_BTN_on && !IS_DOWN_BTN_on) {
        mode++;
        if (mode > NBR_MODES) {
      ...
      } else if (IS_DOWN_BTN_on && !IS_UP_BTN_on) {
        mode--;
        if (mode < 1) {
          mode = 1;
        }
      }
      eeprom_write_byte((EE_ADDR)MODELMODE_EEPROM_OFFSET, mode);

      phase = 3;
      blink = millis() + BLINK_MODE_REPEAT;
      up_or_down_btn_pressed = true;
    } else if(!IS_UP_BTN_on && !IS_DOWN_BTN_on && up_or_down_btn_pressed) {
      up_or_down_btn_pressed = false;
    }
  }
  return mode;
}
...
void update_serial_data()
{
  ...
  else if ( ((rx_ok_buff[1] & 0x80) == 0) && ((cur_protocol[1] & 0x80) != 0) ) // Bind flag has been reset
...
void pollBoot()
{
...
#endif
  {
    ...
    else if ( lState == BOOT_WAIT_20 && rxchar == 0x20 ) // Waiting for 0x20
      lState = BOOT_READY ;
  }
